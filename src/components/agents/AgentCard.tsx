...
function AgentCardComponent({ agent, showAuthor = true }: AgentCardProps) {
  return (
    <Link href={`/agents/${agent.slug}`}>
      <Card className="h-full bg-white hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer active:scale-95">
        <CardHeader>
          ...
          {showAuthor && agent.profile && (
            <div className="flex items-center justify-between pt-4 border-t">
              ...
              <div className="flex items-center gap-1 text-muted-foreground">
                ...
                <span className="text-sm">{agent.favorites_count || 0}</span>
              </div>
            </div>
          )}
        </CardContent>
